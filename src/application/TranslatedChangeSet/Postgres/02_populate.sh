...
#docker exec postgres_container psql --port=5432 --username=postgres --no-password --no-align -c '\connect testdatabase;'
#docker exec postgres_container psql --port=5432 --username=postgres --no-password --no-align -c 'SHOW search_path;'

 
docker exec postgres_container psql --port=5432 --username=postgres --no-password -d testdatabase --no-align -c 'create schema CE;'

#docker exec postgres_container psql --port=5432 --username=postgres --no-password --no-align -c 'SET search_path TO ce;'
...
docker exec postgres_container psql --port=5432 --username=postgres --no-password -d testdatabase --no-align -c 'select * from CE.OPPORTUNITYPOINTSDISCR;'
