...
nohup $JAVA_HOME/bin/java -server -d64 -ms1024M -mx30720M $debug -cp $CLASSPATH -Dlog4j.configuration=log4j.properties $opnet $gcopts $ORACLE_ISSUE_OPTS net.ahm.careengine.cecacheserver.CECacheServer >> ${NOHUP_FILE} 2>&1 &
